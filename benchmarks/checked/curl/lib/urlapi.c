  ...
  if(ptr) {
    *part = mm_strdup_from_raw(ptr);
    ...
    if(urldecode) {
      ...
      /* this unconditional rejection of control bytes is documented
         API behavior */
      CURLcode res = mm_Curl_urldecode(NULL, *part, 0, &decoded, &dlen,
                                    REJECT_CTRL);
      MM_FREE(char, *part);
      if(res) {
        *part = NULL;
                       ...
                       mm_array_ptr<const char> part, unsigned int flags)
{
  ...
  switch(what) {
  case CURLUPART_SCHEME:
    if(mm_strlen(part) > MAX_SCHEME_LEN)
      /* too long */
      return CURLUE_MALFORMED_INPUT;
    if(!(flags & CURLU_NON_SUPPORT_SCHEME) &&
       /* verify that it is a fine scheme */
       !Curl_builtin_scheme(_GETCHARPTR(part)))
      return CURLUE_UNSUPPORTED_SCHEME;
    storep = &u->scheme;
    urlencode = FALSE; /* never */
    break;
  ...
  case CURLUPART_PORT:
  {
    ...
    urlencode = FALSE; /* never */
    port = mm_strtol(part, &endp, 10);  /* Port number must be decimal */
    if((port <= 0) || (port > 0xffff))
      return CURLUE_BAD_PORT_NUMBER;
    if(*endp)
      /* weirdly provided number, not good! */
  ...
  case CURLUPART_URL: {
    ...
    if(Curl_is_absolute_url(_GETCHARPTR(part), NULL, MAX_SCHEME_LEN + 1)) {
      ...
      if(!handle2)
        return CURLUE_OUT_OF_MEMORY;
      result = mm_parseurl(part, handle2, flags);
      if(!result)
        mv_urlhandle(handle2, u);
      else
        curl_url_cleanup(handle2);
  ...
  DEBUGASSERT(storep);
  {
    mm_array_ptr<const char> newp = part;
    size_t nalloc = mm_strlen(part);
    ...
    if(urlencode) {
      mm_array_ptr<const unsigned char> i = NULL;
      mm_array_ptr<char> o = NULL;
      mm_array_ptr<char> enc = MM_ARRAY_ALLOC(char, nalloc * 3 + 1); /* for worst case! */
      if(!enc)
        return CURLUE_OUT_OF_MEMORY;
      for(i = (mm_array_ptr<const unsigned char>)part, o = enc; *i; i++) {
        if((*i == ' ') && plusencode) {
          *o = '+';
          o++;
        }
