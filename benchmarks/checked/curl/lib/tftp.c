                                      ...
                                      const char *ptr, int len)
{
  ...
  while(tmp < ptr + len) {
    ...
    if(checkprefix_raw(option, TFTP_OPTION_BLKSIZE)) {
      long blksize;
    ...
    else if(checkprefix_raw(option, TFTP_OPTION_TSIZE)) {
      long tsize = 0;
                                ...
                                tftp_event_t event)
{
  ...
  case TFTP_EVENT_TIMEOUT: /* Resend the first packet out */
    ...
    /* As RFC3617 describes the separator slash is not actually part of the
       ...
       string. */
    result = Curl_urldecode(data, _GETCHARPTR(&state->data->state.up.path[1]), 0,
                                      ...
                                      struct connectdata *conn)
{
  ...
  type = strstr(_GETCHARPTR(data->state.up.path), ";mode=");
