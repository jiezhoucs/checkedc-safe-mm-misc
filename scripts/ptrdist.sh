...
#
# This scritps run the benchmarks of the original Ptrdist test suite.
#
...
DATA_DIR="$DATA_DIR/ptrdist/checked"
...
#
clean() {
    echo "Cleaning all original Ptrdist benchmark binaries..."
    for prog in ${PROGRAMS[@]}; do
        echo "Cleaning $prog"
        cd "$BIN_DIR/$prog"
        find . -name "*.o" -delete
